...
class SignetResNet(nn.Module):
    def __init__(self, block, num_blocks,
        ...
        if self.adopt_classifier:
            ...
            if False:
                ...
                self.fc = nn.Linear(block_size[3] * block.expansion,
                                    2, bias=False)
    ...
    def forward(self, x, mask=None, mode="train", sparsity=None):
        ...
        x = self.layer4(x, mask, mode, sparsity['layer3'])
    ...
    def forward_without_cf(self, x, mask=None, mode="train", sparsity=None):
        ...
        x = self.layer4(x, mask, mode, sparsity=sparsity)
...
class SigResnet50_cifar_small(nn.Module):
    def __init__(self, num_classes=100, adopt_classifier=True, flatten=True, sparsity=0.1, gamma=0.9):
        super(SigResnet50_cifar_small, self).__init__()

        track = True
        affine = True

        self.func = SignetResNet(SignetBasicBlock, [3, 4, 6, 3],
