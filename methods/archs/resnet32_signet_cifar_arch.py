...
def signet_conv3x3(in_planes, out_planes, stride=1):
    return SignetConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
...
class SigResnet32_cifar_small(nn.Module):
    def __init__(self, num_classes=100, adopt_classifier=True, flatten=True, sparsity=0.1, gamma=0.9):
        super(SigResnet32_cifar_small, self).__init__()

        track = True
        affine = True

        self.func = SignetResNet(SignetBasicBlock, [5, 5, 5],
